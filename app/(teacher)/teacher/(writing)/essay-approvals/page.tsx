...
export default function EssayApprovalsPage() {
    ...
    useEffect(() => {
        const loadPending = async () => {
            try {
                ...
                const allFlags = snap.docs.map(d => ({ id: d.id, ...d.data() } as PendingEssay))
                    .filter(e =>
                ...
                setEssays(allFlags.filter(e => e.status === 'pending_teacher_approval'))
                setArchivedEssays(allFlags.filter(e => e.status !== 'pending_teacher_approval'))
            } catch (err) {
    ...
    return (
        <TeacherLayout title="Essay Approvals">
            <div className="p-4 sm:p-6 md:p-8 max-w-6xl mx-auto">
                <div className="mb-6 md:mb-8">
                    <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-1">Essay Approvals</h1>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {essays.map(essay => (
                                        <tr key={essay.id} className="hover:bg-slate-50 transition-colors">
                                            <td className="px-6 py-4 font-medium text-slate-900">{essay.studentName}</td>
                                            <td className="px-6 py-4 max-w-[200px] truncate text-slate-700">{essay.title}</td>
                                            <td className="px-6 py-4 text-slate-500">{essay.topicName}</td>
                                            <td className="px-6 py-4 text-slate-500">
                                                {essay.submittedAt?.toDate ? essay.submittedAt.toDate().toLocaleDateString() : '—'}
                                            </td>
                ...
                {!loading && archivedEssays.length > 0 && (
                    <div className="mt-12">
                        <h2 className="text-xl font-bold text-slate-900 mb-1">Processed Archive</h2>
                        <p className="text-slate-500 text-sm mb-4">Essays you have already approved or rejected</p>
                                            ...
                                            <tr key={essay.id} className="hover:bg-slate-50 transition-colors">
                                                ...
                                                <td className="px-6 py-4">
                                                    {essay.status === 'rejected' ? (
                                                        <span className="inline-flex items-center px-2.5 py-1 bg-red-50 text-red-600 font-semibold text-xs rounded-full">Rejected</span>
                                                    ) : (
                                                        <span className="inline-flex items-center px-2.5 py-1 bg-green-50 text-green-600 font-semibold text-xs rounded-full">Approved</span>
                                                    )}
                                                </td>
                                                <td className="px-6 py-4 text-slate-500">
                                                    {essay.submittedAt?.toDate ? essay.submittedAt.toDate().toLocaleDateString() : '—'}
                                                ...
                                                <td className="px-6 py-4 text-right">
                                                    <button
                                                        onClick={() => setSelectedEssay(essay)}
                                                        className="inline-flex items-center px-3 py-1.5 bg-slate-100 text-slate-600 font-semibold rounded-lg hover:bg-slate-200 transition-colors"
                                                    >
                                                        View
                                                    </button>
                                                </td>
                            ...
                            {selectedEssay.status === 'pending_teacher_approval' ? (
                                <div className="px-6 pt-4 border-t border-slate-200 bg-white">
                                    ...
                                    <textarea
                                        ...
                                        className="w-full text-sm border border-slate-300 rounded-lg p-2 focus:ring-red-500 focus:border-red-500 outline-none"
                                        rows={2}
                                    />
                                </div>
                                ...
                                {selectedEssay.status === 'pending_teacher_approval' && (
                                    <div className="flex gap-3 w-full sm:w-auto order-1 sm:order-2">
                                        ...
                                        <button
                                            ...
                                            className="flex-1 sm:flex-none px-6 py-2.5 bg-green-600 text-white font-bold rounded-xl hover:bg-green-700 shadow-md transition-all disabled:opacity-50"
                                        >
