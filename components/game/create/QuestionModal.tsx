...
import { Question, QuestionOption, OPTION_COLORS, answersToOptions, optionsToAnswers } from "./types";
...
export default function QuestionModal({ isOpen, onClose, onSave, initialQuestion, globalTimeLimit }: QuestionModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      if (initialQuestion) {
        ...
        setDraftOptions(answersToOptions(initialQuestion.answers));
        setLocalTimeLimit(initialQuestion.timeLimit || globalTimeLimit);
      } else {
        setDraftText("");
        setLocalTimeLimit(globalTimeLimit);
        setDraftOptions([
          { id: '1', text: "", isCorrect: true, colorClass: OPTION_COLORS[0], explanation: '' },
          { id: '2', text: "", isCorrect: false, colorClass: OPTION_COLORS[1], explanation: '' },
          { id: '3', text: "", isCorrect: false, colorClass: OPTION_COLORS[2], explanation: '' },
          { id: '4', text: "", isCorrect: false, colorClass: OPTION_COLORS[3], explanation: '' },
        ]);
      }
    }
  ...
  const handleSave = () => {
    const question: Question = {
      ...
      text: draftText,
      type: initialQuestion?.type || 'quiz',
      timeLimit: localTimeLimit,
      answers: optionsToAnswers(draftOptions),
    };
    onSave(question);
    onClose();
  };
        ...
        <div className="p-4 bg-white flex flex-col gap-4">
          ...
          <div className="flex gap-4">
            ...
            <div className="flex-1 bg-white border-2 border-[#00c0a3] rounded-xl flex items-center justify-center relative shadow-inner">
              <textarea
                className="w-full h-full p-6 text-center text-2xl text-gray-800 placeholder-gray-400 resize-none focus:outline-none"
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute bottom-3 left-3 flex gap-1.5 opacity-70 hover:opacity-100 transition-opacity">
                    ...
                    <div className="border border-white/50 p-1 rounded cursor-pointer hover:bg-white/20 flex items-center justify-center w-[26px]"><span className="text-white font-bold text-xs leading-none">x&sup1;</span></div>
                  </div>
                </div>
              );
            })}
